...
{
	//设置标题
	SetWindowText(TEXT("茶馆游戏中心--够级(不憋三-不进贡)"));
...
bool CGameClientDlg::OnGameMessage(WORD wSubCmdID, const void * pBuffer, WORD wDataSize)
{
	...
	case SUB_S_HAVE_END:		//有玩家出完牌
		{
			return OnHaveEnd(pBuffer,wDataSize);
		}
	}
...
bool CGameClientDlg::OnSubGameStart(const void * pBuffer, WORD wDataSize)
{
	...
	//设置界面
	m_GameClientView.SetCardCount(SwitchViewChairID(pGameStart->wCurrentUser),m_bCardCount[pGameStart->wCurrentUser]);
...
bool CGameClientDlg::OnSubOutCard(const void * pBuffer, WORD wDataSize)
{
	...
	if (m_bCardCount[pOutCard->wOutCardUser]==0)
	{
		...
		{
		case 1:
			strcpy(m_GameClientView.HaveEnd[wOutViewChairID],"头科");
			break;
		case 2:
			strcpy(m_GameClientView.HaveEnd[wOutViewChairID],"二科");
			break;
		case 3:
			strcpy(m_GameClientView.HaveEnd[wOutViewChairID],"三科");
			break;
		case 4:
			strcpy(m_GameClientView.HaveEnd[wOutViewChairID],"三落");
			break;
		case 5:
			strcpy(m_GameClientView.HaveEnd[wOutViewChairID],"二落");
			break;
		case 6:
			strcpy(m_GameClientView.HaveEnd[wOutViewChairID],"大落");
			break;
		}
		if (m_EndOrder>=GAME_PLAYER-1)
			m_EndOrder=0;
	}
	//出牌设置m_EndOrder
...
bool CGameClientDlg::OnSubGameEnd(const void * pBuffer, WORD wDataSize)
{
	...
	m_EndOrder=0;
	memset(m_GameClientView.HaveEnd,0,sizeof(m_GameClientView.HaveEnd));
	return true;
}
...
bool CGameClientDlg::AutomatismOutCard()
{
	...
	if (m_bTurnCardCount==0)
	{
		...
		int i=m_bHandCardCount-1;
	/*	for(;i>0;i--)
		{
			.if (GetCardLogicValue(m_bHandCardCount[i])!=3)
				break;
		}*/
		OutCard.bCardData[0]=m_bHandCardData[i];
...
//有玩家出完牌
bool CGameClientDlg::OnHaveEnd(const void * pBuffer, WORD wDataSize)
{
	return true;
}
//定时器消息
...
/*	if (m_GameLogic.GetCardLogicValue(OutCard.bCardData[0])==3)
	{
		if (OutCard.bCardCount!=m_bHandCardCount)
			return 0;
	}
	*/
	SendData(SUB_C_OUT_CART,&OutCard,sizeof(OutCard)-sizeof(OutCard.bCardData)+OutCard.bCardCount*sizeof(BYTE));
